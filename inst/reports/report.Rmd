...
p1 <- ggplot(roll, aes(date, y = mean_dist)) +
  geom_path(na.rm = T) +
  facet_wrap(~lubridate::month(datetime), scales = 
                             "free_x") +
    labs(title = "Mean Step Size on a Rolling interval" , 
       y = "Mean Step Size (m)")+ theme_minimal()

p2 <- ggplot(roll, aes(date, y = mean_ang)) +
  geom_path(na.rm = T) +
    facet_wrap(~lubridate::month(datetime), scales = 
                             "free_x") +
    labs(title = "Mean Turning Angle on a Rolling interval" , 
       y = "Mean Angle (radians)")  + theme_minimal()
