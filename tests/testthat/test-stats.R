...
AG24 <- dl(AG195) %>% adehabitatLT::subsample(dt = 24, units = "hour") %>% ld %>%
...
test_that("season intervals work", {
  ...
  df$date <- seq(lubridate::ymd_hms("2012-5-15 12:00:00"),
                 lubridate::ymd_hms("2013-5-15 12:00:00"), length.out = 366)
  ...
  expect_equal(nrow(interval_stats(df, type = "seasonal",
                                   seas = c("2012-05-01", "2012-12-01", "2013-06-01"))),2)
})
